  ...
  switch( button_state )
  {
    ...
    case BS_FIRST_PUSH:
	    if( time_out )			// if timeout
	    {
	      ...
	      put_msg_event( SEB_BUTTON_EVENT_MENU_MODE, BE_LONG_PUSH );
	    }
	    else
	    {
    ...
    case BS_FIRST_RELEASE:
	    if( time_out )			// if timeout
	    {
	      ...
	      put_msg_event( SEB_BUTTON_EVENT_MENU_MODE, BE_SINGLE_PUSH );
	      put_msg_event( SEB_BUTTON_EVENT_RTC_ADJUST, BE_SINGLE_PUSH );
	    }
	    else
	    {
    ...
    case BS_SECOND_PUSH:
        ...
        if( ! button_pushed( ) )					// if button released
		    {
	        button_state = BS_IDLE;
	        put_msg_event( SEB_BUTTON_EVENT_RTC_ADJUST, BE_DOUBBLE_PUSH );
        }
	    }
	    break;
    case BS_LONG_PUSH:
